...
const SES_REGION = process.env.SES_REGION; // must match your SES identity region (e.g., eu-central-1)
...
if (PROVIDER === 'ses' && !SES_REGION) {
  throw new Error('[MAIL] SES_REGION is required and must match the region of your SES identity.');
}
...
async function sendEmail(to, subject, html, text, reqId) {
  if (PROVIDER === 'dry-run') {
    console.log(`[MAIL-DRYRUN][${reqId || '-'}] to=${to} subject="${subject}"`);
    const url = extractFirstUrl(html) || extractFirstUrl(text);
    if (url) console.log(`[MAIL-DRYRUN][${reqId || '-'}] url=${url}`);
  ...
  try {
    ...
    console.error(`[MAIL][${reqId || '-'}] send failed`, {
      code: e?.name || e?.Code,
      status,
      message: e?.message
    });
...
If you did not create this account, you can ignore this email.`;
  ...
  <div style="font-family:system-ui,-apple-system,Segoe UI,Roboto,Ubuntu,Arial,sans-serif;color:#e7eaf2;background:#0a0b0d;padding:24px">
    <div style="max-width:520px;margin:auto;background:#121315;border:1px solid #1a1c20;border-radius:10px;padding:20px">
      ...
      <p style="color:#8a93a8">If the button does not work, copy and paste this link:<br><span style="word-break:break-all">${url}</span></p>
    </div>
  </div>`;
