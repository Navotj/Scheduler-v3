...
function normalizeDaysPayload(days) {
  return (Array.isArray(days) ? days : new Array(7).fill([])).map(day =>
    Array.isArray(day)
      ? day.map(it => {
...
// GET /templates  -> REST-style list (alias for /templates/list)
router.get(
  '/',
  requireAuth,
  async (req, res) => {
...
// GET /templates/:id  -> REST-style get by id
router.get(
  '/:id',
  requireAuth,
  param('id').isMongoId(),
  async (req, res) => {
    ...
    const where = { _id: req.params.id, userId: req.user.id };
    const doc = await Template.findOne(where).lean();
    if (!doc) return res.status(404).json({ error: 'not_found' });
    res.json({ template: toClient(doc, true) });
  }
...
router.post(
  ...
  async (req, res) => {
    ...
    const payload = {
      ...
      days: normalizeDaysPayload(req.body.days)
    };
...
// POST /templates  -> REST-style create/upsert by name (alias for /templates/save without id)
router.post(
  '/',
  requireAuth,
  body('name').isString().trim().isLength({ min: 1, max: 80 }),
  ...
  async (req, res) => {
    ...
    let doc = await Template.findOne({ userId, name });
    if (doc) {
      doc.tz = payload.tz;
      doc.stepMin = payload.stepMin;
      doc.hoursStart = payload.hoursStart;
...
// PUT /templates/:id  -> REST-style update
router.put(
  '/:id',
  requireAuth,
  param('id').isMongoId(),
  ...
  async (req, res) => {
    ...
    const doc = await Template.findOne({ _id: id, userId });
    if (!doc) return res.status(404).json({ error: 'not_found' });
...
// DELETE /templates/:id  -> REST-style delete by id
router.delete(
  '/:id',
  requireAuth,
  param('id').isMongoId(),
  async (req, res) => {
    ...
    await Template.deleteOne(where);
    res.json({ success: true });
  }
);

// DELETE /templates  -> delete by id or name via query/body
router.delete(
  '/',
  requireAuth,
  query('id').optional().isMongoId(),
